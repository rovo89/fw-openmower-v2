...
Input* WorxInputDriver::AddInput(const InputConfig& input_config) {
  ZCBOR_REQUIRE_PRESENT(input_config, worx_id);
  decltype(INPUT_BITS)::key_type input_id{to_string_view(input_config.worx_id)};
  auto bit_it = INPUT_BITS.find(input_id);
  if (bit_it == INPUT_BITS.end()) {
    ULOG_ERROR("Unknown Worx input ID %s", input_id);
    return nullptr;
  }

  WorxInput& input = inputs_.emplace_back();
  input.name = to_string_view(input_config.name);
  input.bit = bit_it->second;
  return &input;
}
